...
class AgentPipeline:
    ...
    def __init__(self, model):
        ...
        self.writer_agent = CodeAgent(
            tools=[create_text_file, read_text_file],
            model=model,
        ...
        self.analyzer_agent = CodeAgent(
            tools=[read_text_file, analyze_text_statistics],
            model=model,
        ...
        self.validator_agent = CodeAgent(
            tools=[read_text_file, validate_content],
            model=model,
        ...
        self.reporter_agent = CodeAgent(
            tools=[read_text_file, format_report, create_text_file],
            model=model,
            additional_authorized_imports=[]
        )
    ...
    def run_pipeline(self, topic: str, output_file: str = "final_report.txt"):
        ...
        print("-" * 60)
        analyzer_result = self.analyzer_agent.run(
            "Read 'draft.txt' and analyze its text statistics. Return the statistics."
        )
